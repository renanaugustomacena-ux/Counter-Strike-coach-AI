...
graph LR
    ...
    subgraph "Macena CS2 — Adattamento"
        GS["Game State<br/>25-dim vector"] --> MLP_C["MLP Context Encoder<br/>(2 strati)"]
        GS2["Future State<br/>25-dim vector"] --> MLP_T["MLP Target Encoder<br/>(EMA)"]
...
Il vettore prodotto ha esattamente 25 dimensioni (`METADATA_DIM = 25`), ciascuna normalizzata in un intervallo compatibile con reti neurali:
...
Il passaggio da Vision Transformer su video a MLP su vettore 25-dim comporta perdite e guadagni specifici:

**Cosa si perde:**
- **Relazioni spaziali**: un ViT cattura correlazioni tra patch spaziali tramite self-attention. Un MLP su 25 feature tratta ogni feature come indipendente — non esiste un meccanismo per apprendere che "posizione X + nemico visibile in direzione Y implica rischio di tipo Z".
...
graph TD
    ...
    subgraph "Feature Engineering"
        ...
        FE --> Vec["Vettore 25-dim<br/>[health, armor, ... round_phase]"]
    end
...
L'encoder è il componente che proietta l'input dallo spazio delle feature (25-dim) allo spazio latente (256-dim). La sua architettura è un MLP a due strati con normalizzazione e regolarizzazione:
...
**Analisi dei parametri**: il primo strato `Linear(25, 512)` ha 25×512 + 512 = 13.312 parametri. Il secondo `Linear(512, 256)` ne ha 512×256 + 256 = 131.328. Con LayerNorm (512 + 256 parametri ciascuno per gamma/beta), il totale dell'encoder è circa **145.664 parametri** — ordini di grandezza inferiore a un ViT-Base (86 milioni).

La scelta architetturale è deliberata: con soli 25 feature in input, un ViT sarebbe enormemente sovra-parametrizzato. La bottleneck expansion (19 → 512 → 256) permette una proiezione non-lineare con sufficiente capacità rappresentativa per il dominio tattico.
...
Il `ConceptLabeler` (`jepa_model.py:459-614`) genera etichette soft per i 16 concetti utilizzando **regole euristiche applicate alle stesse 25 feature** che il modello codifica. Questo è un problema di **label leakage** esplicitamente documentato nel codice:
...
graph LR
    ...
    CL["ConceptLabeler<br/>regole euristiche<br/>dalle stesse 25 feature"] -->|"soft labels"| BCE["BCE Loss<br/>(multi-label)"]
    Sim --> BCE
...
- **La JEPA NON riceve i tensori visivi**. Riceve solo il vettore 25-dim dal FeatureExtractor. Non ha alcuna capacità di elaborare immagini o rappresentazioni spaziali 2D/3D.
...
graph TD
    ...
    FE --> Vec["Vettore 25-dim"] --> JEPA["JEPA Model<br/>MLP Encoder"]
...
|---------|------|------|------|
| **Input** | Frame video RGB (H×W×3) | Vettore numerico 25-dim | Perdita totale dell'informazione spaziale/visiva |
| **Encoder** | Vision Transformer (12-40 blocchi, 86M-1B params) | MLP a 2 strati (~146K params) | Capacità rappresentativa ridotta di ~600x |
...
1. **Predizione nello spazio latente**: il predictor opera su embedding 256-dim, non sulle 25 feature grezze
...
graph TB
    ...
    subgraph "Macena CS2 JEPA"
        F1["25-dim Vector<br/>migliaia di tick"] --> MLP1["MLP Encoder<br/>2 strati<br/>~146K params"]
        F2["25-dim Vector<br/>(split temporale)"] --> MLP2["MLP Target<br/>(EMA fisso 0.996)"]
...
Il vettore 25-dim è deterministico, ben normalizzato, configurabile tramite HeuristicConfig, e condiviso coerentemente tra training e inferenza. La codifica ciclica dello yaw e la z_penalty per mappe multi-livello dimostrano attenzione ai dettagli del dominio.
...
4. **Architettura ibrida vision+features**: alimentare la JEPA sia con il vettore 25-dim che con i tensori visivi del TensorFactory, usando encoder multimodali (CNN per i tensori + MLP per le feature, concatenazione nello spazio latente)
...
graph TD
    ...
    subgraph "Feature Layer"
        DB --> FE["FeatureExtractor<br/>25-dim vector"]
...
| `backend/processing/feature_engineering/vectorizer.py` | ~200 | FeatureExtractor, METADATA_DIM=25, extract(), extract_batch() |
