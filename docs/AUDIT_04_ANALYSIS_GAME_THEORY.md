...
| # | Severity | Category | Line(s) | Finding | Recommendation |
|---|----------|----------|---------|---------|----------------|
| A-12 | HIGH | Correctness | 72 | **WinProbabilityNN input_dim=12 vs trainer's 9-dim architecture.** This module defines `WinProbabilityNN(input_dim=12)` with 12 input features (L167-190: 3 economy + 3 player count + 1 utility + 1 map control + 1 time + 1 bomb + 1 side + 1 derived econ ratio). However, the trainer module (`win_probability_trainer.py`, Report 1) instantiates `WinProbabilityNN` with a different feature set (9 dims). A checkpoint saved by the trainer **cannot** be loaded by this predictor — the `Linear(9, 64)` weight matrix is incompatible with `Linear(12, 64)`. Since no checkpoint is typically loaded (W-02 error log at L132), the mismatch is latent but would surface on any attempt to use a trained model. | Align input dimensions: either update the trainer to produce 12-feature vectors, or update this predictor to use 9 features. Both must share a single `_extract_features()` implementation. |
