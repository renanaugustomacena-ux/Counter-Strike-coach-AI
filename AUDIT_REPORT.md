...
## Reconciliation — Top 30 Findings (2026-03-08)

> Cross-reference of every Top 30 finding against remediation Phases 0-12 and DEFERRALS.md.
> Legend: **FIXED** = code change verified | **ACCEPTED** = by design, documented | **OPEN** = still needs work
...
| 4 | `backup_manager.py:65` | CRITICAL | **FIXED** | Phase 1: `text()` wrapper for SQLAlchemy 2.x (F-code in Phase 1 report) |
...
| 13 | `main.py:1548` | HIGH | **FIXED** | Phase 7: `parsing_dialog = None` after dismiss (line 1612) |
| 14 | `main.py:810` | HIGH | **FIXED** | Phase 7: "Section 2 omitted" placeholder removed, real implementation added |
| 15 | `coaching_dialogue.py:297` | HIGH | **ACCEPTED** | By design: `[:-1]` excludes the raw user message just appended; augmented version replaces it (line 306). Not a bug — see F5-06. |
| 16 | `pro_ingest.py:42` | HIGH | **FIXED** | Phase 6: real player names from demo data |
| 17 | `pro_demo_miner.py:98-120` | HIGH | **ACCEPTED** | Knowledge mining uses metadata extraction (not raw demo analysis). Acknowledged as heuristic in DEFERRALS. |
| 18 | `fetch_hltv_stats.py:214` | HIGH | **FIXED** | Phase 6: `_parse_clutches` removed; HLTV pipeline restructured to stats scraping |
| 19 | `config.py:201` | HIGH | **FIXED** | `load_user_settings()` has try/except with fallback defaults (line 186-187). Keyring failures handled gracefully. |
...
| 25 | `training_orchestrator.py:288-300` | HIGH | **FIXED** | Phase 3/12: JEPA validation properly encodes negatives in latent space |
...
| 29 | 4 Alembic migrations NOT NULL | HIGH | **ACCEPTED** | SQLite tolerates ALTER ADD without server_default on existing rows. Migrations work in practice. Documented in DEFERRALS. |
...
The audit covered 391 files across 13+ batches with ~235 total issues (25 CRITICAL, 55 HIGH, 85 MEDIUM, 70 LOW). The remediation Phases 0-12 addressed 368 total issues — more than the original audit count — because additional issues were discovered and fixed during deep code review.
...
- **MONITORING** (12 items tracked in DEFERRALS.md, awaiting production data)
