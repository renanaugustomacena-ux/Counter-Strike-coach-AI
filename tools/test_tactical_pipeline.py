...
def main():
    ...
    # ══════════════════════════════════════════════════════════════
    # STAGE 3-9: Per-map validation
    # ══════════════════════════════════════════════════════════════
    for map_name, (frames, events, segments) in map_data.items():
        _validate_map_data(map_name, frames, events, segments)
        meta = _test_playback_and_spatial(map_name, frames)
        _test_map_image_and_render(map_name, frames, meta)

    _print_summary()


def _validate_map_data(map_name, frames, events, segments):
    """Stages 3-5: validate frames/events/segments structure + sample-frame
    completeness for player and nade fields the Qt map_widget reads."""
...
def _test_playback_and_spatial(map_name, frames):
    """Stages 6-7: PlaybackEngine load+interpolate + SpatialEngine transforms.

    Returns map_metadata (or None when not registered) so the caller can pass
    it into the render simulation.
    """
...
def _test_map_image_and_render(map_name, frames, meta):
    """Stages 8-9: locate map image asset + simulate Qt-free rendering calls."""
    print(f"\n[Stage 8] Map image resolution ({map_name})")
    try:
        from Programma_CS2_RENAN.core.config import get_resource_path

        clean = (
            map_name.lower().strip().replace(".dem", "").replace(".vpk", "").replace("maps/", "")
        )
