...
# ── Phase 25: Design Token Freshness ─────────────────────────────────────────
# JSON at design/tokens/design-tokens.json is the source of truth; Python
# dataclass at Programma_CS2_RENAN/apps/qt_app/core/design_tokens.py is
# generated by tools/gen_design_tokens.py. Fail the gate if a JSON edit
# wasn't re-run through the generator — otherwise screens would render
# stale colors.

print("\n[Phase 25] Design Token Freshness")


def verify_design_tokens_fresh():
    import subprocess

    gen_path = _project_root / "tools" / "gen_design_tokens.py"
    if not gen_path.exists():
        raise RuntimeError(f"token generator missing: {gen_path}")
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(f"design_tokens.py is stale — run the generator. {msg}")


check("Design-Tokens", "core/design_tokens.py in sync with JSON", verify_design_tokens_fresh)


def verify_design_tokens_ts_fresh():
    """Same freshness check for the TypeScript mirror (P4 web marquee)."""
    import subprocess
    ...
    if not gen_path.exists():
        raise RuntimeError(f"token generator missing: {gen_path}")
    result = subprocess.run(
        [sys.executable, str(gen_path), "--web", "--check"],
        capture_output=True,
        text=True,
        cwd=str(_project_root),
        timeout=15,
    ...
    if result.returncode != 0:
        msg = (result.stderr or result.stdout or "").strip()
        raise RuntimeError(f"tokens.ts is stale — run the generator. {msg}")


check(
    "Design-Tokens",
    "web/shared/tokens.ts in sync with JSON",
    verify_design_tokens_ts_fresh,
)


# ── Phase 26: Web Marquee Scaffold Health ────────────────────────────────────
# Read-only structural check — we don't require the app to be built
# (no `pnpm install` needed), just that the scaffold files exist for
# whichever marquee apps we've committed.

print("\n[Phase 26] Web Marquee Scaffold Health")


def _check_web_app_scaffold(app_name: str):
    def _run():
        app_dir = _project_root / "Programma_CS2_RENAN" / "apps" / "qt_app" / "web" / app_name
        if not app_dir.is_dir():
            # Not scaffolded yet — skip by raising a warning.
            raise FileNotFoundError(f"web/{app_name}/ not scaffolded (skip until P4.1+)")
        required = (
            "package.json",
            "tsconfig.json",
            "vite.config.ts",
        ...
        if missing:
            raise RuntimeError(f"web/{app_name}/ scaffold incomplete: {', '.join(missing)}")
...
warn(
    "Web-Marquee",
    "tactical-viewer scaffold complete",
    _check_web_app_scaffold("tactical-viewer"),
)
warn(
    "Web-Marquee",
    "match-detail scaffold complete",
    _check_web_app_scaffold("match-detail"),
)
warn(
    "Web-Marquee",
    "coach-chat scaffold complete",
    _check_web_app_scaffold("coach-chat"),
)


def verify_web_shared_sibling_files():
    shared = _project_root / "Programma_CS2_RENAN" / "apps" / "qt_app" / "web" / "shared"
    if not shared.is_dir():
        raise FileNotFoundError(f"web/shared/ missing: {shared}")
    required = ("tokens.ts", "qwebchannel.ts")
    missing = [f for f in required if not (shared / f).exists()]
    if missing:
        raise RuntimeError(f"web/shared/ incomplete: {', '.join(missing)}")


check(
    "Web-Marquee",
    "web/shared/ has tokens.ts + qwebchannel.ts",
    verify_web_shared_sibling_files,
)
