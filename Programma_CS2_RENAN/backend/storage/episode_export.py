...
Lives in the package (WP4b) so the in-app Train action can call it — the
frozen build ships no ``tools/``; ``tools/export_episodes.py`` is the CLI
wrapper.  Incremental: a demo whose shard already exists in its current
split with the recorded size is reused (``--full`` re-exports everything);
shards of demos that left a split are parked under ``<split>/_stale/``.
...
def open_ro(db_path: Optional[str] = None) -> sqlite3.Connection:
    if db_path is None:
        ...
        db_path = os.path.realpath(DATABASE_URL.replace("sqlite:///", ""))
...
def _git_sha() -> str:
    """Provenance stamp: the git HEAD in a checkout, the app version when frozen
    (no git, and a console-subsystem child would flash a window — WP4b)."""
    ...
    try:
        return subprocess.run(
            ...
            text=True,
            check=True,
            **AppLifecycleManager.hidden_console_kwargs(),
        ).stdout.strip()
...
@dataclass
class ExportSummary:
    """What one export run produced (and reused)."""
    ...
    fingerprint: str = ""  # digest of the train split's shard hashes


def _load_existing_manifests(out_dir: Path) -> Dict[str, Dict[str, Dict[str, Any]]]:
    """``{split: {demo_name: manifest entry}}`` for every split dir with a manifest."""
    ...
    for split in ("train", "val", "test"):
        ...
        try:
            ...
            _log(f"  unreadable manifest {manifest_path}: {exc} — split re-exported")
            continue
        existing[split] = {d["demo_name"]: d for d in manifest.get("demos", []) if "demo_name" in d}
    return existing


def _reusable_entry(out_dir: Path, split: str, entry: Dict[str, Any]) -> bool:
    """A shard is reused when it exists in its split with the recorded size
    (older manifests without a size fall back to the sha256)."""
    path = out_dir / split / entry.get("file", "")
    if not path.is_file():
        return False
    size = entry.get("size")
    if size:
...
def _park_stale_shards(out_dir: Path, split: str, keep: set) -> int:
    """Move shards of demos that left ``split`` to ``<split>/_stale/`` — never
    deleted, because ``ShardIndex`` globs the split dir (the sampler must not
    see a demo the DB no longer assigns there)."""
    split_dir = out_dir / split
    if not split_dir.is_dir():
        return 0
    moved = 0
    for path in split_dir.glob("*.safetensors"):
        if path.name in keep:
            continue
        stale_dir = split_dir / "_stale"
        ...
        moved += 1
    if moved:
        _log(f"  {split}: parked {moved} stale shard(s) under {split_dir / '_stale'}")
    return moved
...
def export_episodes(
    ...
    *,
    ...
    """In-process entry point (the training pipeline): same as the CLI."""
...
def run_export(args: argparse.Namespace) -> ExportSummary:
    out_dir = Path(args.out)
    out_dir.mkdir(parents=True, exist_ok=True)
    full = bool(getattr(args, "full", False))
    existing = {} if full else _load_existing_manifests(out_dir)
    ...
    for demo in demo_names:
        ...
        if split is None:
            ...
            continue
        entry = existing.get(split, {}).get(demo)
        if entry is not None and _reusable_entry(out_dir, split, entry):
            split_demos.setdefault(split, []).append(dict(entry))
            reused += 1
            continue
        tasks.append((demo, split, pms_rows))

    _log(
        f"Export: {len(tasks)} demos ({reused} reused, {skipped_unassigned} skipped unassigned), "
    ...
    def finish(split: str, result: Dict[str, Any], seconds: float) -> None:
        ...
        entry = {
            ...
            "file": fname,
            "sha256": sha,
            "size": (out_dir / split / fname).stat().st_size,
    ...
    # Every split that has (or had) a manifest is rewritten, so a split that
    # lost all its demos ends with an empty manifest instead of a stale one.
    stale = 0
    for split in sorted(set(split_demos) | set(existing) | {"train", "val", "test"}):
        demos = split_demos.get(split, [])
        if not demos and not (out_dir / split).is_dir():
            continue
        _write_manifest(out_dir, split, demos, split_warnings.get(split, []), args.seed)
        stale += _park_stale_shards(out_dir, split, {d["file"] for d in demos})
    ...
    _log(
        f"Done: {total_shards} shards ({reused} reused), {total_episodes} episodes, "
        f"{total_ticks} ticks, {skipped_empty} empty demos, "
        f"{skipped_unassigned} unassigned, {stale} stale parked"
    )
    train_shas = sorted(e["sha256"] for e in split_demos.get("train", []))
    fingerprint = hashlib.sha256("".join(train_shas).encode("utf-8")).hexdigest()[:16]
    return ExportSummary(
        ...
        stale=stale,
        by_split={split: len(d) for split, d in split_demos.items() if d},
        fingerprint=fingerprint if train_shas else "",
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--full", action="store_true", help="Re-export every demo (default: reuse unchanged shards)"
    )
