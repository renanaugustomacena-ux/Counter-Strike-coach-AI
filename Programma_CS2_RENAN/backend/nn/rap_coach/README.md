...
# RAP Coach — 6-Layer Recurrent Architecture

RAP (Recurrent Architecture for Pedagogy) Coach implements a 6-layer neural architecture with LTC-Hopfield memory for tactical coaching.
...
### Layer 6: Role Classification
- **NeuralRoleHead** (in `model.py`) — 5-class role classifier (Entry/Lurk/Support/AWP/IGL) with consensus mechanism

## Supporting Modules

- **model.py** — `RAPCoachModel` — Full RAP Coach orchestration integrating all 6 layers
- **trainer.py** — `RAPTrainer` — Training orchestration with callback support for TensorBoard/Observatory
- **chronovisor_scanner.py** — `ChronovisorScanner`, `CriticalMoment` — Multi-scale critical moment detection (micro/standard/macro timescales)
