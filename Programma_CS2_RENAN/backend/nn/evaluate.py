...
def evaluate_adjustments(model, X_sample, role_id=None):
    ...
    # 2. Explanation (SHAP)
    # Use a small background dataset for KernelExplainer if available,
    # or a dummy background for this specific evaluation
    def model_wrapper(x):
    ...
    return {
        ...
        "explanations": shap_values,  # SHAP values for each output
    }
