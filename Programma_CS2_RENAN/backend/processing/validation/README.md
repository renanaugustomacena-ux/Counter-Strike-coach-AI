...
.dem file lands in the ingest folder
    ...
    |     - kills / deaths / assists / adr / headshot_pct / kast bounds (LIMITS)
...
- **Structured logging.** `drift.py`, `sanity.py`, and `schema.py` log via `get_logger("cs2analyzer.<module>")`; `dem_validator.py` performs no logging and instead returns an explicit `(is_valid, game_version, error_message)` verdict.
...
5. Provide a unit test under `Programma_CS2_RENAN/tests/` (e.g. `test_dem_validator.py`, `test_drift_and_heuristics.py`).
