...
def debug_save_page_source(start_date: date, end_date: date):
    ...
    app_logger.info("[DEBUG] Fetching URL for debugging: %s", url)
...
def _execute_page_save(page, browser, url):
    try:
        app_logger.info("[DEBUG] Navigating to page...")
        ...
        app_logger.info("[DEBUG] Closing browser.")
        browser.close()
...
def _write_content(content):
    ...
    with open(DEBUG_HTML_PATH, "w", encoding="utf-8") as f:
        f.write(content)
    app_logger.info("[SUCCESS] Saved page HTML for debugging to: %s", DEBUG_HTML_PATH)


def _handle_save_error(page, e):
    app_logger.error("[ERROR] An error occurred during the debug save process: %s", e)
    try:
        with open(DEBUG_HTML_PATH, "w", encoding="utf-8") as f:
            f.write(page.content())
        app_logger.info("[INFO] Saved partial page content on error to: %s", DEBUG_HTML_PATH)
    except Exception as e:
        app_logger.warning("Failed to save partial page content: %s", e)


if __name__ == "__main__":
    app_logger.info("[DEBUG] --- Running simple Playwright page source debug script ---")
    year = 2023
