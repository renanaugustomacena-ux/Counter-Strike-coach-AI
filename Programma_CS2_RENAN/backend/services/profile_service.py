...
def _execute_steam_fetch(key, steam_id):
    try:
        resp = requests.get(
...
def _parse_steam_response(resp, key, steam_id):
...
def _fetch_cs2_hours(key, steam_id):
    r = requests.get(
...
def _execute_faceit_fetch(key, nickname):
