...
Bundle de upload achatado do design atlas para a aplicação de coaching CS2 — uma cópia autocontida dos 41 frames SVG mais os design tokens, dispostos de forma plana para fácil drag-and-drop em ferramentas de design. Todos os arquivos são SVG + HTML + JSON — zero binários, totalmente portátil. O atlas canônico e estruturado reside em `design/` (raiz do repositório); veja `design/README.md`.

## Mapa de arquivos

```
design/cs2/uploads/
├── index.html                  ← cópia do canvas do atlas (veja nota Browser abaixo)
├── README.md
├── README-8252c0ae.md          ← cópia carregada do README raiz do projeto
├── design-tokens.json          ← formato W3C DTCG, 3 temas — cópia idêntica
│                                 do SSOT de tokens (design/tokens/design-tokens.json)
...
Todos os 41 frames são 1440×900. Os SVGs standalone de arquitetura maiores (`system_map.svg`, `jepa_model.svg`, `data_pipeline.svg`) **não** fazem parte deste bundle — residem em `design/architecture/`.

## Como usar

### Browser (Claude Design / preview local)

O `index.html` aqui é uma cópia verbatim do canvas do atlas: ele referencia os SVGs via caminhos relativos `frames/…` e `architecture/…`, que não se resolvem nesta pasta plana. Abra o canvas canônico em vez disso:

```bash
# abre o canvas master (atlas estruturado)
open design/index.html
# ou
firefox design/index.html
```

Funciona como `file://` — sem necessidade de servidor. Carregue `design/index.html` como artifact do Claude para uso interativo; os SVGs individuais nesta pasta abrem diretamente em qualquer browser.
...
3. Para tokens: instale o plugin **Figma Tokens** e carregue `design-tokens.json`.
4. Frames de arquitetura (`21`–`30`) são colados em resolução total — as camadas ficam legíveis.

### After Effects

1. **File → Import → File** — selecione qualquer `.svg` numerado desta pasta.
2. O AE importa o SVG como **vector shape layers** — totalmente animáveis.
3. Use os SVGs maiores em `design/architecture/` para seções de vídeo explicativo (viewBox maior = mais espaço).
4. SVGs de diagramas com setas são bons candidatos para animações de **motion path**.
...
2. Mapeie os tokens de cor: valores de `design-tokens.json` → variáveis do Framer.
...
Insira os frames como hero `<img src="01_cover.svg">` — renderização SVG nativa do browser.
...
Três temas disponíveis em `design-tokens.json`:
...
Nota: os tokens e frames deste bundle correspondem ao SSOT atual (`design/tokens/design-tokens.json`): `#FF6A00` sobre `#0B1628`.
...
## SVGs de arquitetura — Onde residem

Os três documentos técnicos standalone (`system_map.svg` 1920×1200, `jepa_model.svg` 1440×1080, `data_pipeline.svg` 1440×900) **não** estão neste bundle — residem em `design/architecture/` na raiz do repositório. A cobertura de arquitetura deste bundle consiste nos frames numerados `21`–`30` (mais os deep-dives RAP `37`–`41`).
...
| `METADATA_DIM=25` | Fonte única: `vectorizer.py` |

## Matriz de compatibilidade

| Ferramenta | Como |
|------|-----|
| Claude Design (web) | Carregue `design/index.html` como artifact; cole SVGs individuais |
