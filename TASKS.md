...
| R6 | Design/decisioni owner | ✅ DECISE 2026-07-17 (owner): #61 26-SCHEMA-02 → **RIMUOVERE** i campi connect-state (feature morta; ri-progettare se/quando servirà, con split DM-02) · C11 26-NORM-01 → **SSOT tick-rate PRE-R8** (accessor unico metadata→header fallback + test anti-letterale-64) · #62 → **cablare eslint 9 flat-config** pinnato nel workspace pnpm · transformers 5.x → **in finestra R8** (bump + re-embedding RAG + EMBEDDING_VERSION insieme al retrain, sulla macchina dove girerà R8). Esecuzione: ✅ #61/C11/#62 ESEGUITI 2026-07-17 stesso giorno (dettagli righe 61/62; C11 = SSOT `core/tick_rate.py` + alias legacy constants.TICK_RATE + 10 siti ri-puntati + test contrattuale AST `test_tick_rate_ssot.py` che vieta i letterali — ha scovato 3 siti extra oltre il censimento manuale). Residuo R6: bump transformers 5.x in finestra R8 | eseguite (residuo: transformers→R8) |
...
| # | Status | Pri | Title |
|---|---|---|---|
| 67 | TODO | MED | **Consolidamento layout tool/test (finestra R10, decisione owner 2026-07-17)** — oggi tool e test vivono in DUE alberi (root `tools/`+`tests/` vs `Programma_CS2_RENAN/tools/`+`tests/`) con regimi di protezione diversi: il manifest di integrità hasha solo il package, il conftest col gate `CS2_INTEGRATION_TESTS` copre solo la suite package (causa radice della marcitura del tier integration, #66/F5), e i tool di repair sono sparsi su entrambi. PR deliberato in R10: (a) unica dir tool con sottocartelle `repair/`/`audit/`/`build/`, (b) test root sotto il conftest gated, (c) manifest + compile-gate + CI + CLAUDE.md/REFERENCE.md aggiornati al nuovo layout, (d) archiviare i 10 one-shot esauriti censiti in #66. NON prima di R8 (churn di path su tutti i gate). |
| 66 | DONE | HIGH | **Staleness audit test+tool (ordine owner 2026-07-17)** — censimento completo post-cambiamenti (orphan sweep, drop connect-state, SSOT tick-rate). TOOLS (68 script): 0 ROTTI · 2 stale-pericolosi FIXATI (mine_shard_strategies: default shards-dir sul mount morto "New Volume"→DP-06 + finestra flash-assist inline a 64 tick→`FLASH_ASSIST_WINDOW_S×rd.tick_rate`; tick_census: idx 2/3 has_helmet/has_defuser ora censiti — il commento "not in monolith" era falso post-repair) · 10 one-shot esauriti (candidati archivio, NON toccati: migrate_db, purge_default_stats_rag, repair_kast/ratings/equipment_value/tick_features, merge_demo_pool, migrate_hltv_schema, repair_rating_scale, seed_hltv_top20) · rebuild_monolith: +3 indici mancanti nel ciclo drop/recreate. TEST (F1-F10): **F1 CRITICO FIXATO** — test NN-MEM-01 importava classe MAI esistita (RecurrentBeliefState) e skippava da sempre come "optional dep"; la produzione marcava `_hopfield_trained=True` anche su load parziale → guardia vera in `RAPMemory.load_state_dict` (trained solo se i pesi hopfield sono nel checkpoint) + 2 test reali · F2 dry-run stubbava `_fetch_rap_ticks` (cancellato R4) → `_fetch_*_windows` · F3 TestBackupMonolith de-skippato (premesse decadute: pytest-timeout pinnato, Online Backup API bounded) su DB temp · F4 path "New Volume" hardcodati in 3 test integration → DP-06 · F5 **il tier `-m integration` (41 test) non aveva NESSUN esecutore** (né CI né locale — causa radice di F1/F2/F4): aggiunto step pytest gated in build.yml job integration · F6 assert Steam/FACEIT → pro-only · F8 riferimento riga stale · F9 forensic_parser_test mai collezionato → rinominato+SSOT · F10 docstring fixture onesto. Residuo: F7 marker `slow` a zero usi (cosmetico); 10 one-shot da archiviare a discrezione owner. |
...
| 38 | DONE | MED | HLTV rescrape — ✅ CHIUSO 2026-08-03/04 (vedi appendice notte): gate sbloccato dall'owner, `rescrape_placeholder_pros --apply` eseguito (placeholder 24→72 per tornei nuovi, 0 FAIL), acceptance PASSATA (`purge_default_stats_rag --dry-run` → 0 placeholder cards). Riga allineata 2026-08-21 (era rimasta TODO per drift del ledger). |
...
| 64 | TODO | MED | **26-RANGE-01 — residuo SOLO design (R9). Meccanica chiusa 2026-08-21 (sweep 15f):** guard ValueError loud GIÀ shipped in `train_jepa_finetune`; fixture 25-out (combinazione impossibile in produzione) ripuntate a OUTPUT_DIM=10 e il test che green-lightava il contratto sbagliato invertito (10-dim y passa, 25-dim deve alzare); contract test nuovo pinna `jepa_insight_adapter._TARGET_FEATURES == vectorizer.FEATURE_NAMES[:10]` (risolto il conflitto documentale con MATCH_AGGREGATE_FEATURES[0:10], che alimenta AdvancedCoachNN via `_calculate_deltas` — modello DIVERSO); F-0029: l'adapter ora rifiuta checkpoint pretrain-only senza `head_trained=True` nel sidecar. **VERIFICA ST-1b FATTA 2026-07-16 (vedi AUDIT §12.1), resta la decisione design (c).** Esito: il finetune CLI reale è **shape-broken** (modello `output_dim=10` WR-63 vs target last-tick **[N,25]** da `load_user_match_sequences` → MSELoss RuntimeError: il path non può girare, nessun checkpoint mai prodotto); i test lo mascherano (fixture `output_dim=25`). I delta [-1,1] di `_calculate_deltas` alimentano SOLO AdvancedCoachNN (tanh — LOSS-02 coerente là); la head JEPA non ha MAI avuto un contratto target. Adapter F1 difensivo + gate 26-HYB-01 → oggi nessuna magnitudine fabbricata. **Fix (pre-requisito R9):** contratto proposto = delta 10-assi `(δ+1)/2` → [0,1], 0.5 neutro per costruzione; CLI deve fallire chiaro nel frattempo; test di contratto su config reale 25→10. |
...
| 62 | DONE | LOW | ✅ FATTO 2026-07-17 (decisione owner: cablare): root manifest privato del workspace + `eslint.config.mjs` flat unica (eslint 9.39.5, typescript-eslint 8.64.0, globals 17.7.0 — pinnati); `pnpm -r lint` VERDE su tutti e 3 gli app. Bonus root-cause: store pnpm del box conteneva file root-owned (install sudo di aprile) e pnpm 9 ignora le chiavi config in pnpm-workspace.yaml (supporto = pnpm 10, quella era la sessione Windows) → store rigenerato pulito. |
| 61 | DONE | MED | ✅ FATTO 2026-07-17 (decisione owner: RIMUOVERE): campi steam_connected/faceit_connected tolti da Ext_PlayerPlaystyle (db_models) + migrazione idempotente `f6a7b8c9d0e1` APPLICATA al monolite (colonne droppate); check_prerequisites ora esplicitamente pro-only (il ramo user-demo era irraggiungibile dal giorno uno — escisso, messaggi onesti senza 'Connect IDs'); test flows aggiornati (53 verdi). Reintroduzione solo con design completo (writer+reader+split DM-02). |
...
| 65 | DONE | LOW | **26-DEP-03 — ✅ CHIUSO 2026-08-03 (PR #46, riga «#65» della sessione):** pin sostituito con `hopfield-layers @ git+...@f56f929` (stesso commit dei lock) in `requirements-rap.txt`. Riga allineata 2026-08-21 (era rimasta TODO per drift del ledger). |
...
| 28.2 | DONE | LOW | `core/session_engine.py` | 20→0 | ✅ 2026-08-21: solo 6 siti broad restavano (il censimento "20" era stale — heartbeat→OSError e state-writes→(SQLAlchemyError,OSError) erano già stretti da campagne precedenti). Chiusura: `_monitor_stdin` → `(OSError, ValueError)`; i 4 top-of-daemon (keep-alive, Scanner, Digester, Teacher) restano broad BY DESIGN (crash-contain) e lo scan-cycle interno resta broad documentato (boundary demoparser2 — classe F-0006). Nessun sito "worker spawn subprocess" esiste: i daemon sono thread. |
| 28.3 | DONE | LOW | `backend/data_sources/demo_parser.py` | 3→0 | ✅ Già soddisfatto da F-0006 (audit 2026-08-14): i 3 siti sono `except BaseException` + `is_parse_error` (parse_guard SSOT) DI PROPOSITO — demoparser2/PyO3 non esporta eccezioni tipizzate (`DemoParserException` NON esiste; verificato sul .pyi 0.41.4) e la PanicException pyo3 nasce lazy. Restringere qui sarebbe una REGRESSIONE (riaprirebbe il buco F-0006). Interrupt propagano via `_PROPAGATE`. |
| 28.4 | DONE | LOW | `core/lifecycle.py` | 3→0 | ✅ 2026-08-21: spawn/kill/reap erano GIÀ stretti alle tuple prescritte (`(OSError, ValueError, subprocess.SubprocessError)` / `(OSError, ProcessLookupError, subprocess.SubprocessError)`). Chiusi i 2 residui reali: acquire/release del lock single-instance POSIX → `(ImportError, OSError)` (LockConflict già separato). |
